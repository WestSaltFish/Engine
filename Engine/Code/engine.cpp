...
GLuint FindVAO(Mesh& mesh, u32 submeshIndex, const Program& program)
{
	...
	if (ret == 0)
	{
		...
		glBindBuffer(GL_ARRAY_BUFFER, mesh.indexBufferHandle);
		...
		for (auto ShaderIt = ShaderLayout.cbegin(); ShaderIt != ShaderLayout.cend(); ++ShaderIt)
		{
			...
			for (auto SubmeshIt = SubMeshLayout.cbegin(); SubmeshIt < SubMeshLayout.cend(); ++SubmeshIt)
			{
				if (ShaderIt->location == SubmeshIt->location)
				{
					...
					const u32 offset = SubmeshIt->offset;
					const u32 stride = submesh.vertexBufferLayout.stride;

					glVertexAttribPointer(index, ncomp, GL_FLAT, GL_FALSE, stride, (void*)(u64)(offset));
		...
		ModelLoader::VAO vao = { ret,program.handle };
...
void Init(App* app)
{
	...
	//vertexBufferLayout.stride = 5 * sizeof(float); // x,y,z,u,v 
...
void Render(App* app)
{
	...
	case Mode_TexturedQuad:
	{
		...
		const Program& textureMeshProgram = app->programs[app->texturedMeshProgramIdx];
		glUseProgram(textureMeshProgram.handle);

		Model& model = app->models[app->patricioModel];
		Mesh& mesh = app->meshes[model.meshIndex];
		...
		for (u32 i = 0; i < mesh.submeshes.size(); ++i)
		{
			...
			u32 subMeshMaterialIndex = model.materialIndex[i];
			Material& subMeshMaterial = app->materials[subMeshMaterialIndex];

			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_2D, app->textures[subMeshMaterial.albedoTextureIndex].handle);
			glUniform1i(app->texturedMeshProgram_uTexture, 0);

			SubMesh& submesh = mesh.submeshes[i];
			glDrawElements(GL_TRIANGLES, submesh.indices.size(), GL_UNSIGNED_INT, (void*)(u8)submesh.indexOffset);
		}
	}
	break;
